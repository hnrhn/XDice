    ...
    public void onMessageCreate(MessageCreateEvent event) {
        ...
        String response = switch (command.getCommandType()) {
            ...
            case FK_ROLL -> {
                responsePrefix = mention;
                try {
                    yield ((FKRoller)dependencies.getDiceRoller()).fkRoll(command.getFallenKingdomsDice(), DIContainer.getRepository(), Long.toString(dependencies.getConfigId()) + dependencies.getUserId());
                } catch (SQLException e) {
                    e.printStackTrace();
                    yield "ERROR";
                }
            ...
            case FK_PUSH -> {
                responsePrefix = mention;
                try {
                    yield ((FKRoller)dependencies.getDiceRoller()).fkPush(Long.toString(dependencies.getConfigId()) + dependencies.getUserId(), DIContainer.getRepository(), false);
                } catch (SQLException e) {
                    e.printStackTrace();
                    yield "ERROR";
                }
            ...
            case FK_PRIDE -> {
                responsePrefix = mention;
                try {
                    yield ((FKRoller)dependencies.getDiceRoller()).fkPush(Long.toString(dependencies.getConfigId()) + dependencies.getUserId(), DIContainer.getRepository(), true);
                } catch (SQLException e) {
                    e.printStackTrace();
                    yield "ERROR";
                }
            }
            case FK_CREATE_NEW_DECK -> {
                responsePrefix = mention;
                try {
                    yield ((FKRoller)dependencies.getDiceRoller()).fkNewDeck(dependencies.getConfigId(), DIContainer.getRepository());
                } catch (SQLException e) {
                    e.printStackTrace();
                    yield "ERROR";
                }
            }
            case FK_DRAW_INITIATIVE -> {
                responsePrefix = mention;
                try {
                    yield ((FKRoller)dependencies.getDiceRoller()).fkDrawInitiative(dependencies.getConfigId(), DIContainer.getRepository());
                } catch (SQLException e) {
                    e.printStackTrace();
                    yield "ERROR";
                }
