...
public class MessageListener implements MessageCreateListener {
    ...
    public void onMessageCreate(MessageCreateEvent event) {
        ...
        try {
            dependencies = new DIContainer(event.getServer().orElseThrow().getIdAsString(), event.getMessageAuthor().getIdAsString());
        ...
        String response = switch (command.getCommandType()) {
            ...
            case FL_ROLL -> {
                responsePrefix = mention;
                try {
                    yield ((FLRoller)dependencies.getDiceRoller()).flRoll(command.getForbiddenLandsDice(), DIContainer.getRepository(), dependencies.getConfigId() + dependencies.getUserId());
                } catch (SQLException e) {
                    e.printStackTrace();
                    yield "ERROR";
                }
            ...
            case FL_PUSH -> {
                responsePrefix = mention;
                try {
                    yield ((FLRoller)dependencies.getDiceRoller()).flPush(dependencies.getConfigId() + dependencies.getUserId(), DIContainer.getRepository(), false);
                } catch (SQLException e) {
                    e.printStackTrace();
                    yield "ERROR";
                }
            ...
            case FL_PRIDE -> {
                responsePrefix = mention;
                try {
                    yield ((FLRoller)dependencies.getDiceRoller()).flPush(dependencies.getConfigId() + dependencies.getUserId(), DIContainer.getRepository(), true);
                } catch (SQLException e) {
                    e.printStackTrace();
                    yield "ERROR";
                }
